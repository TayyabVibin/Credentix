...
import { fetchAdminWebhooks } from '../../store/adminSlice.ts';

export default function AdminWebhooksPage() {
  const [page, setPage] = useState(1);
  const dispatch = useAppDispatch();
  const { webhookLogs, webhookPage, webhookPages, loading } =
    ...
    <PageTransition>
      <Box>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          Delivery logs from Adyen webhooks
        </Typography>
              ...
              {loading ? (
                ...
                webhookLogs.map((log) => (
                  <TableRow key={log.id} hover>
                    ...
                    <TableCell>
                      <Chip label={log.eventCode} size="small" variant="outlined" />
                    </TableCell>
                    <TableCell>
                      {log.success ? (
