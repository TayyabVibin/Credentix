    ...
    <PageTransition>
      <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#0A0C10', p: 2, position: 'relative', overflow: 'hidden' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ width: '100%', maxWidth: 480, position: 'relative', zIndex: 1 }}>
          <Card sx={{ backdropFilter: 'blur(20px)', bgcolor: 'rgba(20,23,31,0.7)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <CardContent sx={{ p: 4 }}>
              <Box sx={{ textAlign: 'center', mb: 1 }}>
                <Typography variant="h4" fontWeight={800} sx={{ background: gradients.hero, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Credentix</Typography>
              </Box>
              <Box sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography variant="caption" sx={{ color: '#94A3B8' }}>Create your account</Typography>
                </Box>
                <LinearProgress variant="determinate" value={progress} sx={{ height: 4, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.06)', '& .MuiLinearProgress-bar': { borderRadius: 2, background: gradients.hero } }} />
              </Box>
              ...
              <AnimatePresence mode="wait">
                <motion.div key="step-0" variants={stepVariant} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }}>
                  <Typography variant="h6" fontWeight={600} sx={{ mb: 0.5, color: '#F1F5F9' }}>Create your account</Typography>
                  <Typography variant="body2" sx={{ color: '#94A3B8', mb: 3 }}>Start with your email and a secure password</Typography>
                  ...
                  <TextField label="Password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="new-password" helperText="Must be at least 8 characters" slotProps={{ input: { startAdornment: <InputAdornment position="start"><LockRounded sx={{ color: '#64748B' }} /></InputAdornment>, endAdornment: <InputAdornment position="end"><IconButton onClick={() => setShowPassword(!showPassword)} edge="end">{showPassword ? <VisibilityOffRounded /> : <VisibilityRounded />}</IconButton></InputAdornment> } }} sx={{ mb: 2.5 }} />
                  <TextField label="Confirm Password" type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required autoComplete="new-password" slotProps={{ input: { startAdornment: <InputAdornment position="start"><LockRounded sx={{ color: '#64748B' }} /></InputAdornment> } }} />
                </motion.div>
              </AnimatePresence>
