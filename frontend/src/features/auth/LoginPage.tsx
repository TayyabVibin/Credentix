        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 8, position: 'relative', zIndex: 1, maxWidth: 900, width: '100%' }}>
          <Box sx={{ flex: 1, display: { xs: 'none', md: 'block' } }}>
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
              ...
              <Typography variant="h5" fontWeight={700} sx={{ color: 'text.primary', mt: 3, mb: 1 }}>Welcome back to Credentix</Typography>
              <Typography variant="body2" sx={{ color: 'text.secondary', lineHeight: 1.7 }}>Manage your credits, process payments, and monitor transactions — all from one dashboard.</Typography>
            </motion.div>
          </Box>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} style={{ flex: 1, maxWidth: 440 }}>
            <Card sx={{ width: '100%', backdropFilter: 'blur(20px)', bgcolor: palette.mode === 'dark' ? 'rgba(26,29,36,0.8)' : 'rgba(255,255,255,0.9)', border: '1px solid', borderColor: 'divider' }}>
              <CardContent sx={{ p: 4 }}>
                <Box sx={{ textAlign: 'center', mb: 4, color: 'primary.main' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1.5, mb: 1 }}>
                    <CredentixLogo size={36} color="currentColor" />
                    <Typography variant="h4" fontWeight={800} sx={{ color: 'primary.main' }}>Credentix</Typography>
                  </Box>
                ...
                <Typography sx={{ mt: 3, textAlign: 'center' }} variant="body2">
                  <Typography component="span" sx={{ color: 'text.secondary' }}>Don't have an account? </Typography>
